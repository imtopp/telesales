...
class LocationMappingController extends BaseController
{
  ...
  public function index(){
    return view('backend/content/courier/location_mapping');
  }
  ...
  public function read(){
    ...
    if( !empty($requestData['search']['value']) ) {
      ...
      $query = DB::table('courier_location_mapping')
                ->select('courier_location_mapping.id', 'location_province.name AS province', 'location_province.id AS province_id', 'location_city.name AS city', 'location_city.id AS city_id', 'location_district.name AS district','location_district.id AS district_id', 'courier_package.name AS courier_package', 'courier_package.id AS courier_package_id', 'courier.name AS courier', 'courier.id AS courier_id', 'courier_location_mapping.status')
                ->join('location_district','location_district.id','=','courier_location_mapping.location_district_id')
                ->join('location_city','location_city.id','=','location_district.city_id')
                ->join('location_province','location_province.id','=','location_city.province_id')
    ...
    foreach($query as $row) {  // preparing an array
        ...
        $nestedData['action'] = '<td><center>
                           <a href="#" data-id="'.$row->id.'" data-province_id="'.$row->province_id.'" data-city_id="'.$row->city_id.'" data-district_id="'.$row->district_id.'" data-courier_package_id="'.$row->courier_package_id.'" data-courier_id="'.$row->courier_id.'" data-status="'.$row->status.'" data-toggle="tooltip" title="Edit" class="btn btn-sm btn-warning edit" onClick="edit(this)"> <i class="fa fa-pencil"></i> </a>
                           <a href="#" data-id="'.$row->id.'" data-name="<span style='."'font-size: small';>".'<br/>Provinsi : '.$row->province.'<br/>Kota : '.$row->city.'<br/>Kecamatan : '.$row->district.'<br/>Kurir : '.$row->courier.'<br/>Paket Pengiriman : '.$row->courier_package.'</span>" data-toggle="tooltip" title="Hapus" class="btn btn-sm btn-danger destroy" onClick="destroy(this)"> <i class="fa fa-trash"></i> </a>
                           </center></td>';
  ...
  public function create(){
    ...
    $modelExists = LocationMappingModel::where(['location_district_id'=>$_POST['district_id'],'courier_package_id'=>$_POST['courier_package_id']])->count();
    if(!$modelExists){
      $model = new LocationMappingModel;
      $model->location_district_id = $_POST['district_id'];
      $model->courier_package_id = $_POST['courier_package_id'];
  ...
  public function getCourier(){
    return CourierModel::join('courier_package','courier_package.courier_id','=','courier.id')
                        ->lists('courier.name','courier.id');
  }

  public function getCourierPackage(){
    if(isset($_POST['courier_id']))
      $courier_package = CourierPackageModel::where(['courier_id'=>$_POST['courier_id']])->lists('name','id');
    else
