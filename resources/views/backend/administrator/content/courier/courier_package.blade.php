...
@section('page-js-script')
  ...
  var table;
    $(document).ready(function() {
      ...
      table = $('#datatable').dataTable({
        ...
        ajax: {
          ...
          url: "{{URL::route('administrator_manage_courier_package_read')}}",
          type: "POST",
    ...
    function initializeModal(mode,element,title,id,name,courier_id,status){
      $.spin("show");
      ...
      if(mode == "create" || mode == "update"){
          $(function(){
            $.ajax({
              url : '{{URL::route('administrator_manage_courier_package_get_courier')}}',
              type: 'POST',
              dataType: 'JSON',
              data: {"_token":"{{ csrf_token() }}"},
    ...
    function create(e) {
      ...
      setSubmitModalEvent('{{URL::route('administrator_manage_courier_package_create')}}');
    }

    function edit(e) {
      initializeModal("update",$("#modal-template").html(),"Edit Data",$(e).data('id'),$(e).data('name'),$(e).data('courier_id'),$(e).data('status'));

      setSubmitModalEvent('{{URL::route('administrator_manage_courier_package_update')}}');
    }

    function destroy(e) {
      initializeModal("delete",$("#modal-template-delete").html(),"Delete Data",$(e).data('id'),$(e).data('name'));

      setSubmitModalEvent('{{URL::route('administrator_manage_courier_package_destroy')}}');
    }
  </script>
@stop
