...
<head>
  ...
  <title>{{ config('settings.app_name') }} | Product Details</title>
...
<body>
      ...
      <div id="product_detail row">
        ...
        <div class="left-side col-xs-12 col-md-5">
          <div class="img_preview row" style="margin-bottom: 10px; margin-top: 10px;">
            ...
            <div class="col-md-10">
              <img id="img_preview" src="{{$product['colours'][0]['image_url']}}" style="max-width:100%; max-height:360pt" onerror="this.onerror=null;this.src='assets/img/img_not_found.jpg';"/>
            </div>
          </div>
          <hr>
          <div class="img_thumb row">
            <ul class="img-nav">
              ...
              <li>
                <a class="img-nav-icon" id="thumbnail_{{$colour['fg_code']}}" data-id="{{$colour['id']}}" data-fg-code="{{$colour['fg_code']}}" data-price="{{$colour['price']}}" href="{{$colour['image_url']}}">
        ...
        <div class="right-side col-xs-12 col-md-5">
          ...
          {!! Form::open(array('route'=>'order','class'=>'form','method'=>'get')) !!}
          <div class='form-group'>
            {!! Form::label('Variant') !!}
            {!! Form::select('colour_id',isset($product['colours'])?collect($product['colours'])->lists('name','fg_code'):array(),$product['colours'][0]['fg_code'],array('required','id'=>'colour_id','class'=>'form-control','placeholder'=>'Harap Pilih Varian Product')) !!}
          </div>
          <h2 id="price_label" style="margin-top: 20px; font-family: 'Roboto-bold', sans-serif;">Rp {{ number_format($product['colours'][0]['price'],0,",",".") }}</h2>
          <div class="form-group">
  ...
  $(document).ready(function(){
    ...
    var colour_id  = $("#thumbnail_"+$("#colour_id").val());
    var price = colour_id.data("price");
  ...
  $(".img-nav-icon").click(function(e){
    ...
    $("#price_label").html("Rp "+price.formatMoney(0, ',', '.'));
    $("#price").val(price);
    $("#colour_id").val($(this).data("fg-code"));
